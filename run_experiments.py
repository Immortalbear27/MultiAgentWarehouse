...
def single_run(params):
    ...
    # If an internal RNG is used elsewhere:
...
def run_batch(params_batch):
    """
    Execute a batch of parameter configurations by running `single_run` for each.

    Processes a list of parameter dictionaries, collecting each successful
    output or capturing exceptions as error entries.

    Args: List of parameter sets to pass to `single_run`.

    Returns: Results from `single_run` for each parameter set. If `single_run`
    raises an exception, the returned entry is the original params merged
...
def chunk_list(lst, n_chunks):
    """
    Split a list into a specified number of roughly equal-sized chunks.

    Calculates the chunk size so that all elements are distributed
    across `n_chunks`, with the final chunk possibly smaller.

    Args:
    lst: The list to be partitioned.
    n_chunks: Desired number of chunks.

    Returns: A list containing `n_chunks` sublists.
    """
